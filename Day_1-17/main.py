...
def median_self(data):    #对于自己手写的案例函数，函数名加“_self”
    """中位数"""
    temp,data_len = sorted(data), len(data)
    if data_len % 2 == 0:
        return mean(temp[data_len // 2 - 1:data_len // 2 +1 ])  #4/2 = 2.0 not integer
    else:
        return temp[data_len // 2]
...
"""
#day17 的递归函数还需要实战，目前看来需要尽快完成100days学习，进而实战验证；
